...
from firebase.config import db, APP_VERSION
...
class DashboardApp(QMainWindow):
    ...
    def __init__(self, username, user_data, company_name: str = "ERP"):
        ...
        self.maint_watcher = None

        self._build_ui()
        self._start_network_monitor()
        self._start_maintenance_watcher()
        self._check_maintenance_once()
    ...
    def _build_ui(self):
        ...
        sidebar_items = [
            ...
            ("Manufacturing", [
                ("Powder Coating", lambda: self.launch_module("powdercoating_window", PowderCoatingMain, self.user_data)),
            ...
            ("Core Options", [
                ...
                ("Manage Users (Admin Only)", lambda: setattr(self, "view_users_window", ViewUsersModule.show_if_admin(self.user_data))),
        ...
        self.sidebar = create_expandable_sidebar(self, sidebar_items, self.logout, font_scale=1.1)
        ...
        # Quick actions
        if self._is_admin:
            self.btn_maintenance = QToolButton()
            self.btn_maintenance.setCursor(Qt.PointingHandCursor)
            self.btn_maintenance.setCheckable(True)
            self.btn_maintenance.clicked.connect(self._toggle_maintenance_mode)

            # Initialize from whatever we currently know; will be re-synced by watcher and one-shot check
            self._sync_maintenance_button(self._maintenance_active)

            h.addWidget(self.btn_maintenance)
        ...
        if self._is_admin:
            ...
            self.nonadmin_launchpad = self._build_nonadmin_launchpad()
            right.addWidget(self.nonadmin_launchpad)

            # Maintenance panel (hidden until needed)
            self.maintenance_panel = QFrame()
            self.maintenance_panel.setObjectName("Card")
            lay_m = QVBoxLayout(self.maintenance_panel)
            lay_m.setContentsMargins(24, 32, 24, 32)
            lbl = QLabel("🛠 Under maintenance")
            lbl.setAlignment(Qt.AlignCenter)
            lbl.setStyleSheet("font-size:22px; font-weight:900; color:#111827;")
            sub = QLabel("Thank you for your patience.")
            sub.setAlignment(Qt.AlignCenter)
            sub.setStyleSheet("font-size:16px; font-weight:600; color:#6b7280;")
            lay_m.addWidget(lbl)
            lay_m.addWidget(sub)
            self.maintenance_panel.hide()
            right.addWidget(self.maintenance_panel)
        right.addStretch(1)
    ...
    def _build_nonadmin_launchpad(self):
        ...
        message_content.setText(f"• Please contact your admin if you have any issues accessing the modules.\n• Keep this section updated.")
    ...
    def _on_net_status(self, status: str, rtt_ms: int):
        ...
        if status == "offline":
            ...
            try:
                if self.maint_watcher:
                    self.maint_watcher.stop()
            except Exception:
                pass
        elif status in ("online", "slow"):
            self._exit_offline_mode()
            # Re-subscribe realtime listener
            try:
                if self.maint_watcher:
                    self.maint_watcher.start()
            except Exception:
                pass
            # One manual check to ensure state is current
            self._check_maintenance_once()
    ...
    def resizeEvent(self, event):
        ...
        try:
            ...
            if hasattr(self, "version_label"):
                margin = 12
                w, h = self.version_label.width(), self.version_label.height()
    ...
    def _start_maintenance_watcher(self):
        try:
            self.maint_watcher = MaintenanceWatcher(self)
            self.maint_watcher.maintenance_changed.connect(self._on_maintenance_changed)
            self.maint_watcher.start()
        except Exception:
            self.maint_watcher = None
            
    def _check_maintenance_once(self):
        """Perform a single Firestore read to sync current maintenance state."""
        try:
            doc = db.collection("meta").document("maintenance_mode").get()
            if doc.exists:
                d = doc.to_dict() or {}
                state = bool(d.get("status", False))
                self._on_maintenance_changed(state)
        except Exception:
            pass

    def _on_maintenance_changed(self, is_active: bool):
        self._maintenance_active = bool(is_active)
        
        # NEW: keep the header button in sync for admins
        if self._is_admin:
            self._sync_maintenance_button(self._maintenance_active)
        ...
        if not self._is_admin:
            if self._maintenance_active:
                ...
                # 2) hide sidebar entirely (like no allowed modules)
                try: self.sidebar.hide()
                except Exception: pass

                # 3) show maintenance message, hide normal non-admin launchpad
                try:
                    if hasattr(self, "nonadmin_launchpad") and self.nonadmin_launchpad:
                        self.nonadmin_launchpad.hide()
                ...
                try:
                    if hasattr(self, "maintenance_panel") and self.maintenance_panel:
                        self.maintenance_panel.hide()
                    if hasattr(self, "nonadmin_launchpad") and self.nonadmin_launchpad:
                        self.nonadmin_launchpad.show()
                    self.sidebar.show()
                except Exception:
                    pass
                
    # --- Maintenance button helpers (NEW) ---
    def _toggle_maintenance_mode(self):
        """Toggles maintenance mode on/off for all users."""
        if not hasattr(self, "btn_maintenance") or not self.btn_maintenance:
            return
        active = self.btn_maintenance.isChecked()
        try:
            # Persist in Firestore
            db.collection("meta").document("maintenance_mode").set({"status": active}, merge=True)
            # Mirror local state and visuals
            self._maintenance_active = active
            self._update_maintenance_button_style(active)
            QMessageBox.information(self, "Maintenance Mode",
                                    "Maintenance mode enabled" if active else "Maintenance mode disabled")
        except Exception as e:
            # Revert UI if write failed
            self._sync_maintenance_button(not active)
            QMessageBox.critical(self, "Error", f"Failed to update maintenance mode:\n{e}")

    def _update_maintenance_button_style(self, active: bool):
        """Updates button label and style according to maintenance state."""
        if not hasattr(self, "btn_maintenance") or not self.btn_maintenance:
            return
        if active:
            self.btn_maintenance.setText("Maintenance On")
            self.btn_maintenance.setStyleSheet("""
                ...
                QToolButton:hover { background-color: #4ade80; }
            """)
            self.btn_maintenance.setToolTip("Click to turn OFF maintenance mode")
        else:
            self.btn_maintenance.setText("Maintenance Off")
            self.btn_maintenance.setStyleSheet("""
                QToolButton {
            ...
            """)
            self.btn_maintenance.setToolTip("Click to turn ON maintenance mode")

    def _sync_maintenance_button(self, active: bool):
        """Programmatically mirror Firestore state into the header button without emitting signals."""
        if not hasattr(self, "btn_maintenance") or not self.btn_maintenance:
            return
        try:
            self.btn_maintenance.blockSignals(True)
            self.btn_maintenance.setChecked(active)
            self._update_maintenance_button_style(active)
        finally:
            self.btn_maintenance.blockSignals(False)

    def _set_maintenance_button_enabled(self, enabled: bool):
        """Enable/disable the button (used for offline handling)."""
        if hasattr(self, "btn_maintenance") and self.btn_maintenance:
            self.btn_maintenance.setEnabled(enabled)
            if not enabled:
                self.btn_maintenance.setToolTip("Disabled while offline")

    # ---------------- Module management ----------------
    def launch_module(self, attr_name, window_class, *args):
        if self._maintenance_active and not self._is_admin:
    ...
    def closeEvent(self, event):
        ...
        try:
            if self.monitor and self.monitor.isRunning():
                self.monitor.terminate()  # or self.monitor.stop() if implemented
